...
from dust3r.POMATO import AsymmetricMASt3R
...
def get_args_parser():
    ...
    parser.add_argument("--server_port", type=int, help=("will start gradio app on this port (if available). "
                        ...
                        default=None)
    # parser.add_argument("--weights", type=str, help="path to the model weights", default='/home/spyder/project/perception/mast3r/checkpoint/train_head3_wosup/7datasets_wosup_10epochs.pth')
    parser.add_argument("--weights", type=str, help="path to the model weights", default='/home/spyder/project/perception/mast3r/checkpoint/train_head3_matching_PO/5datasets_v2.pth')

    # TODO: update model name here for pretrained model loading
    ...
    parser.add_argument('--moving_distance_thres_global', type=float, default=0.3, help='Moving distance threshold for flow loss in global alignment')
    parser.add_argument('--flow_loss_weight', type=float, default=0.01, help='Flow loss weight for global alignment')
    parser.add_argument('--moving_distance_thres_adjacent', type=float, default=1.2, help='Moving distance threshold for seperating moving regions in adjacent pairs')
...
def get_reconstructed_scene(args, outdir, model, device, silent, image_size, filelist, schedule, niter, min_conf_thr,
                            ...
                            flow_loss_weight, flow_loss_start_iter, flow_loss_threshold, moving_distance_thres_global, moving_distance_thres_adjacent, use_gt_mask, fps, num_frames):
    """
    ...
    if new_model_weights != args.weights:
        model = AsymmetricMASt3R.from_pretrained(new_model_weights).to(device)
    model.eval()
    if seq_name != "NULL":
    ...
    if len(imgs) > 2:
        ...
        scene = global_aligner(output, device=device, mode=mode, verbose=not silent, shared_focal = shared_focal, temporal_smoothing_weight=temporal_smoothing_weight, translation_weight=translation_weight,
                               flow_loss_weight=flow_loss_weight, flow_loss_start_epoch=flow_loss_start_iter, flow_loss_thre=flow_loss_threshold, use_self_mask=not use_gt_mask, moving_distance_thres_global=moving_distance_thres_global, 
                               moving_distance_thres_adjacent=moving_distance_thres_adjacent, num_total_iter=niter, empty_cache= len(filelist) > 72)
    else:
        mode = GlobalAlignerMode.PairViewer
        scene = global_aligner(output, device=device, mode=mode, verbose=not silent)
    lr = 0.01

    # init the camera intrinisics here
    # init_intrinsics = scene._cal_pred_intrinsic()
    K = np.array([[756, 0, 144],
                  [0, 756, 256],
                  [0, 0, 1]])
    # scene.preset_intrinsics([K], [True]*scene.n_imgs)
...
if __name__ == '__main__':
    ...
    if args.input_dir is not None:
        ...
        scene, outfile, imgs = recon_fun(
            ...
            moving_distance_thres_global=args.moving_distance_thres_global,
            moving_distance_thres_adjacent=args.moving_distance_thres_adjacent,
