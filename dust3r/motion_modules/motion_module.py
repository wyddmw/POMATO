...
# sys.path.append('/home/spyder/project/perception/Mastr3r_Matching/mast3r/motion_modules/')
...
class TemporalModule(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.frame_length = temporal_max_len
    ...
    def forward(self, input_tensor, encoder_hidden_states=None, attention_mask=None, ):
        ...
        '''
        input_tensor = rearrange(input_tensor, '(b f) c h w -> b c f h w', f=self.frame_length)
...
if __name__ == '__main__':
    B, T, L, C = 1, 12, 64, 256
    H, W = 48, 64
    fake_data = torch.randn((B*T, C, H, W)).cuda()   # [b*f, c, h, w]
    motion_module = TemporalModule(in_channels=C, temporal_max_len=T).cuda()
    output = motion_module(fake_data, encoder_hidden_states=None)
    import torch.optim as optim
    optimizer = optim.SGD(motion_module.parameters(), lr=0.01)  # Stochastic Gradient Descent
    target = torch.ones_like(output).cuda()
