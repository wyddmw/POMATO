...
import numpy as np
import torch
from dust3r.utils.vo_eval import load_traj, eval_metrics, plot_trajectory, save_trajectory_tum_format, process_directory, calculate_averages, update_timestamps
...
def eval_pose_estimation_dist(args, model, device, img_path, save_dir=None, mask_path=None):
    ...
    for seq in tqdm(seq_list):
        try:
            ...
            if args.pose_end_frame > 0:
                filelist = filelist[:args.pose_end_frame]
            ...
            with torch.enable_grad():
                if len(imgs) > 2:
                    ...
                    scene = global_aligner(
                        ...
                        num_total_iter=args.n_iter, temporal_smoothing_weight=args.temporal_smoothing_weight, moving_distance_thres_adjacent=args.moving_distance_thres_adjacent,
                        moving_distance_thres_global=args.moving_distance_thres_global, flow_loss_start_epoch=args.flow_loss_start_epoch, flow_loss_thre=args.flow_loss_thre,  adjacent_intervals=args.adjacent_intervals,
                        translation_weight=args.translation_weight, sintel_ckpt=args.eval_dataset == 'sintel', use_self_mask=not args.use_gt_mask, sam2_mask_refine=args.sam2_mask_refine,
            ...
            gt_traj = load_traj(gt_traj_file=gt_traj_file, stride=args.pose_eval_stride, traj_format=traj_format, end=args.pose_end_frame)

            # if args.eval_dataset == 'sintel':
            #     gt_traj = load_traj(gt_traj_file=gt_traj_file, stride=args.pose_eval_stride, end=args.pose_end_frame)
            # elif traj_format is not None:
            #     gt_traj = load_traj(gt_traj_file=gt_traj_file, traj_format=traj_format, end=args.pose_end_frame)
            # else:
            #     gt_traj = None
            # import pdb; pdb.set_trace()
            # # update timestamps
            # if traj_format is not None and traj_format in ["tum", "tartanair"]:
            #     pred_traj[1] = update_timestamps(
            #     gt_traj_file, traj_format, stride=args.pose_eval_stride, end=args.pose_end_frame
            # )
            # pdb.set_trace()
