...
def load_traj(gt_traj_file, traj_format="sintel", skip=0, stride=1, end=0):
    ...
    if end > 0:
        traj_tum = traj_tum[:end]
        timestamps_mat = timestamps_mat[:end]
    return traj_tum, timestamps_mat


def update_timestamps(gt_file, traj_format, skip=0, stride=1, end=0):
    ...
    if traj_format == "tum":
        ...
        timestamps = timestamps[skip::stride]
        if end > 0:
